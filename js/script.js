const ApiKey = "5accb29230af4b379f015417c78064ce";
...
function getListTeams() {
    ...
    fetch(teamEndPoint, fetchHeader)
        ...
        .then(resJson => {
            ...
            detil.forEach(btn => {
                btn.onclick = (event) => {
                    getDetailTeam(event.target.dataset.id);
                }
            })
        }).catch(err => {
            console.error(err);
    ...
    fetch(url, fetchHeader)
        ...
        .then(detail => {
            let details = `
                <div class="row">
                    <div class="col s6">
                        <img src="${detail.crestUrl}" alt="Club Image" style="margin-top:20px;" width="250px">
                    </div>
                        <div class="col s6">
                            <p> Nama Tim : ${detail.name}</p>
                            <p> Alamat Tim : ${detail.address}</p>
                            <p> Stadion Kandang : ${detail.venue}</p>
                            <p> Email : ${detail.email}</p>
                            <p> Nomor Telephone : ${detail.phone}</p>
                            <p> Alamat Website : ${detail.website}</p>
                            <p> Tahun Didirikan : ${detail.founded}</p>
                        </div>
                </div>
                `;
